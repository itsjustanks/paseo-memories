...
- Each place is saved and reported on its own: "Saved to X, but not to Y (why)". A new Claude note whose list entry couldn't be written is taken back out, so Claude never has a note it can't find.
- Text copied while a password was hidden (••••) can't be saved as a note; it says to press Show first.
- Note cards never show a file's header (Claude's `paths:`, Copilot's `applyTo:`) as a card, and every Change or Remove keeps it exactly. Edits keep line endings, indented code and line breaks as they were.
...
- **Codex's own working files** (raw memories, rollout summaries, extensions, the working diff, its database, instructions in its settings) stay out of lists, Overview counts, "Worth a look" and search in the plain view. Technical details shows them.
